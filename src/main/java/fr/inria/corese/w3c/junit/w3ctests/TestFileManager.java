 ...
 * This utility class provides methods to handle local storage of remote test resources,
 * ensuring they are up-to-date and providing utility functions for file path manipulation.
 */
public class TestFileManager {

    private static final Logger logger = LoggerFactory.getLogger(TestFileManager.class);
    /**
     * The base path string for test resources. Files downloaded or accessed by this manager
    ...
    /**
     * Sets the update mode flag for the TestFileManager.
     * When update mode is enabled, the manager will check if local files are
     * outdated compared to their remote counterparts and re-download them if necessary.
     *
     * @param updateMode A boolean value: {@code true} to enable update mode, {@code false} to disable it.
     */
     ...
     * it checks if the local file is identical to the remote file based on their hash.
     * If they are different (or the file doesn't exist locally), the local file is replaced.
     *
     * @param fileUri The URI of the file to load.
     * @throws IOException If an I/O error occurs during file operations (e.g., download, read, write).
     * @throws NoSuchAlgorithmException If the SHA-256 hashing algorithm is not available for file comparison.
     */
     ...
     *
     * @param remoteFileUri The remote URI that can be used to determine the local path of the file.
    ...
    /**
     * Compares two files to check if they are different based on their SHA-256 hash.
     * A temporary file is downloaded for the remote URI to perform the hash comparison.
     *
     ...
     * It ensures the parent directories of the local path exist before copying the file.
     *
    ...
    /**
     * Extracts the file name from a {@link Path}.
     *
     * @param filePath The {@link Path} to the file.
     * @return The file name as a {@code String}.
     */
    ...
    /**
     * Extracts the file name from a file path represented as a {@code String}.
     *
     * @param filePath The file path as a {@code String}.
     * @return The file name as a {@code String}.
     */
    ...
    /**
     * Extracts the last two or one segments from the URI path, excluding the actual filename.
     * This is used to create a prefixed folder structure for local caching.
     * For example, for "http://example.org/tests/data/file.ttl", it might return "data/file.ttl".
     *
     * @param uri The URI from which to extract segments.
     * @return A string representing the last relevant path segments, or an empty string if not enough segments.
     */
    ...
    /**
     * Generates a prefixed filename for local storage based on the remote file's URI.
     * This method combines relevant path segments from the URI with the actual filename
